...
final class MilitaryOperationalRoleCatalog
{
    ...
    public static function entries(): array
    {
        ...
        $add($rows, 'staff_joint_coordination_officer', 'Officier coordination interarmes', 'Joint Fires Coordinator', 'État-major', 'Opérations', 'Coordination des effets interarmes et appuis.', 'liaison', 0, $em, 80, 80, 'officer');
        ...
        $add($rows, 'staff_sustainment_lead', 'Responsable soutien', 'Supply Specialist', 'État-major', 'Logistique', 'Gestion des stocks et du soutien quotidien.', 'support', 0, $em, 140, 140, 'logistics');
        $add($rows, 'staff_logistics_flow_manager', 'Gestionnaire flux logistiques', 'Motor Transport Operator', 'État-major', 'Logistique', 'Organisation des flux, convois et dotations.', 'support', 0, $em, 150, 150, 'logistics');
        ...
        $add($rows, 'infantry_radio_operator', 'Opérateur radio', 'Radio Operator', 'Infanterie', 'Spécialités', 'Transmissions et liaisons tactiques.', 'liaison', 0, $inf, 90, 90, 'rto');
        ...
        $fires = 50;
        $add($rows, 'fires_jtac', 'JTAC', 'JTAC', 'Appuis & feux', 'Coordination', 'Contrôleur d’attaques au sol.', 'liaison', 0, $fires, 10, 10, 'officer');
        $add($rows, 'fires_forward_observer', 'Forward Observer', 'Forward Observer', 'Appuis & feux', 'Coordination', 'Observation et ajustement des tirs.', 'liaison', 0, $fires, 20, 20, 'member');
        $add($rows, 'fires_support_officer', 'Officier appuis feux', 'Fire Support Officer', 'Appuis & feux', 'Coordination', 'Synthèse et coordination des appuis.', 'liaison', 0, $fires, 30, 30, 'officer');
        ...
        $add($rows, 'engineer_infra_technician', 'Technicien infrastructure', 'Construction Engineer', 'Génie', 'Infrastructure', 'Travaux d’infrastructure et ouvrages.', 'support', 0, $eng, 40, 40, 'member');
        $add($rows, 'engineer_works_lead', 'Responsable travaux', 'Works Supervisor', 'Génie', 'Infrastructure', 'Pilotage des chantiers et contrôle qualité.', 'support', 0, $eng, 50, 50, 'officer');

        $log = 70;
        $add($rows, 'logistics_driver', 'Conducteur militaire', 'Motor Transport Operator', 'Logistique', 'Transport', 'Conduite et manœuvre des véhicules logistiques.', 'support', 0, $log, 10, 10, 'member');
        $add($rows, 'logistics_convoy_chief', 'Chef convoi', 'Convoy Commander', 'Logistique', 'Transport', 'Responsabilité d’un convoi ou d’un détachement roulant.', 'support', 0, $log, 20, 20, 'officer');

        $add($rows, 'logistics_mechanic', 'Mécanicien', 'Mechanic', 'Logistique', 'Maintenance', 'Maintenance de premier et second échelon.', 'support', 0, $log, 30, 30, 'logistics');
        $add($rows, 'logistics_maint_technician', 'Technicien maintenance', 'Maintenance Technician', 'Logistique', 'Maintenance', 'Diagnostic et réparation des systèmes.', 'support', 0, $log, 40, 40, 'logistics');
        $add($rows, 'logistics_fleet_manager', 'Responsable parc matériel', 'Fleet Manager', 'Logistique', 'Maintenance', 'Gestion du parc et disponibilité opérationnelle.', 'support', 0, $log, 50, 50, 'logistics');
        ...
        $add($rows, 'medical_auxiliary', 'Auxiliaire sanitaire', 'Medical Assistant', 'Santé', 'Médical', 'Soutien sanitaire et assistance au poste de secours.', 'support', 0, $med, 30, 30, 'medic');
        $add($rows, 'medical_first_responder', 'Secouriste', 'Combat Medic', 'Santé', 'Médical', 'Premiers secours et évacuation sanitaire initiale.', 'support', 0, $med, 40, 40, 'medic');
        ...
        $adm = 100;
        $add($rows, 'hr', 'Gestionnaire RH', 'Human Resources Specialist', 'Administration', 'Gestion', 'Gestion des effectifs et du dossier personnel.', 'support', 0, $adm, 10, 10, 'hr');
        $add($rows, 'admin_staff_officer', 'Officier administratif', 'Administrative Officer', 'Administration', 'Gestion', 'Courrier, dossiers et formalités administratives.', 'support', 0, $adm, 20, 20, 'officer');
        $add($rows, 'admin_unit_secretary', 'Secrétaire unité', 'Unit Secretary', 'Administration', 'Gestion', 'Secrétariat et suivi administratif de l’unité.', 'support', 0, $adm, 30, 30, 'member');
