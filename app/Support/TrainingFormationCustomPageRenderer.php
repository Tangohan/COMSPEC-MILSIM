...
final class TrainingFormationCustomPageRenderer
{
    ...
    public static function render(array $row, string $assetsBaseUrl, ?string $chrome = null, ?string $downloadPdfUrl = null, ?string $feedbackHtml = null): string
    {
        ...
        $isPreview = $chrome !== null && $chrome !== '';

        if ($sections !== []) {
            $body = self::renderHandbook(
                $titleEsc,
                $subtitle,
                $summary,
                $intro,
            ...
            $body = self::renderSingleFragment(
                $titleEsc,
                $subtitle,
                $summary,
                $intro,
    ...
    private static function renderSingleFragment(
        string $titleEsc,
        string $subtitle,
        string $summary,
        string $innerHtml,
        ...
        return '<div class="formation-doc-shell formation-doc-shell--single" style="--doc-accent:' . htmlspecialchars($accent, ENT_QUOTES, 'UTF-8') . '">'
            . self::learnerTopbar($catalogueUrl, $isPreview)
            ...
            . $headMeta
            . '</div></header>'
            . '<main class="formation-doc-main"><article class="formation-doc-panel formation-doc-prose">' . $innerHtml . '</article></main>'
            . '</div>';
    }

    /** @param list<array{title:string,slug:string,html:string}> $sections */
    private static function renderHandbook(
        string $titleEsc,
        string $subtitle,
        string $summary,
        string $introHtml,
        ...
        $toc = '';
        if ($showToc) {
            $toc = '<nav class="formation-doc-toc" aria-label="Sommaire">'
                . '<p class="formation-doc-toc__label">Sommaire</p>'
                . '<p class="formation-doc-toc__count">' . $chapterCount . ' chapitre' . ($chapterCount > 1 ? 's' : '') . '</p>'
                . '<ol class="formation-doc-toc__list">';
        ...
        if ($introHtml !== '') {
            $main .= '<section class="formation-doc-panel formation-doc-prose formation-doc-intro" aria-label="Introduction">' . $introHtml . '</section>';
        }
        foreach ($sections as $idx => $s) {
            $slug = htmlspecialchars($s['slug'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
            $t = htmlspecialchars($s['title'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
            $main .= '<article id="' . $slug . '" class="formation-doc-chapter formation-doc-panel">'
    ...
    private static function learnerTopbar(string $catalogueUrl, bool $isPreview): string
    {
        if ($isPreview) {
            return '';
        }
        $href = htmlspecialchars($catalogueUrl, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');

        return '<div class="formation-doc-topbar">'
            . '<div class="formation-doc-topbar__inner">'
            . '<a class="formation-doc-topbar__brand" href="' . $href . '">'
            . '<span class="formation-doc-topbar__mark" aria-hidden="true">A</span>'
            . '<span class="formation-doc-topbar__brand-text">Athena · Formations</span>'
            . '</a>'
            . '<a class="formation-doc-topbar__back" href="' . $href . '">'
    ...
    private static function headerMeta(string $subtitle, string $summary, int $readTime, ?int $chapterCount): string
    {
        ...
        $chips = [];
        $chips[] = '<span class="formation-doc-chip">' . htmlspecialchars($readTime . ' min de lecture', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</span>';
        if ($chapterCount !== null) {
            $chips[] = '<span class="formation-doc-chip">' . htmlspecialchars($chapterCount . ' chapitre' . ($chapterCount > 1 ? 's' : ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</span>';
        }
        $out .= '<div class="formation-doc-metarow">' . implode('', $chips) . '</div>';
