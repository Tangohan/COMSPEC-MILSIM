...
private _signatures = _data getOrDefault ["signatures",createHashMap];
private _signature = str _value;
if ((_signatures getOrDefault [_domain,""]) isEqualTo _signature) exitWith { false };
...
_signatures set [_domain,_signature];
