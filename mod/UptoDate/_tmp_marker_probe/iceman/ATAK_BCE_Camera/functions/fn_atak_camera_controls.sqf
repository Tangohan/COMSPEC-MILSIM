...
switch _ID do {
  ...
  case 2: {
    ...
    if (_state && {!isNull _vehicle} && {_camIndex > -1}) then {
      [{
        ...
        if (!isNull _cam && {_FOV != (_cam_Var param [5, -1])}) then {
          _cam_Var set [5,_FOV];
      ...
      }, {}, [focusOn, cTabUAVcams # _camIndex # 1, _camIndex]] call CBA_fnc_waitUntilAndExecute;
    };
  };
  ...
  case 5: {
    private _state = !(uiNamespace getVariable ["BCE_ATAK_VideoFeed_FullATAK", false]);
    uiNamespace setVariable ["BCE_ATAK_VideoFeed_FullATAK", _state];
    if (_state) then {
