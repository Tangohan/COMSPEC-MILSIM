/*
    ...
    Indépendant de la position BFT : un canal authentifié suffit.
    Si la liaison est momentanément saturée (fiab. dégradée), le marqueur
    est mis en file plutôt que jeté.
*/
params [
    ["_markerName", "", [""]],
    ["_deleted", false, [true]],
...
// Soft-block liaison : ne pas jeter — file d’attente (flush périodique).
private _txBlocked = false;
private _txReason = "";
if (!_force) then {
    private _txGate = [true] call comspec_overwatch_connect_fnc_canTransmit;
    if !(_txGate getOrDefault ["can_transmit", true]) then {
        _txBlocked = true;
        _txReason = _txGate getOrDefault ["reason", "tx_blocked"];
...
private _fnc_dispatch = {
    ...
    if (_blocked || {!(missionNamespace getVariable ["COMSPEC_AthenaReady", false])}) exitWith {
        [_name, _body, _del] call comspec_overwatch_connect_fnc_queueMapMarker;
        true
    };
    private _flag = if (_del) then { "1" } else { "0" };
    "COMSPECExtension" callExtension ["SendMarker", [_name, _body, "1", _flag]];
    missionNamespace setVariable ["COMSPEC_LastMarkerSyncName", _name, false];
