...
if (_hasAce) then {
    ...
    // lifeState INCAPACITATED seul : trop bruyant au spawn / JIP (faux KO).
    // On le garde seulement si ACE signale déjà un état critique OU rythme à zéro.
    if (!_uncon && {lifeState _unit == "INCAPACITATED"}) then {
        if (_hr <= 20 || {_blood < 50} || {_unit getVariable ["ace_medical_inCardiacArrest", false]}) then {
            _uncon = true;
        };
    };
