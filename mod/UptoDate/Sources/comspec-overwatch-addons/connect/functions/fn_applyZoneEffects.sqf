...
if (isNil "_zone" && {!isNil "_lastZone"}) then {
    ...
    if ((missionNamespace getVariable ["COMSPEC_LinkState", "linked"]) isEqualTo "offline") then {
        // Ne pas forcer linked si un crash appareil est en cours
        private _atak = missionNamespace getVariable ["COMSPEC_AtakState", createHashMap];
        if (!(_atak getOrDefault ["device_crashed", false])) then {
...
if (!isNil "_zone") then {
    ...
    if (_forceDisconnect) then {
        ...
        if (_lossFloor >= 35 || {_latencyAdd >= 300}) then {
            missionNamespace setVariable ["COMSPEC_LinkState", "degraded", false];
        } else {
            if ((missionNamespace getVariable ["COMSPEC_LinkState", "linked"]) in ["offline", "degraded"]) then {
                private _atak = missionNamespace getVariable ["COMSPEC_AtakState", createHashMap];
                if (!(_atak getOrDefault ["device_crashed", false])) then {
                    missionNamespace setVariable ["COMSPEC_LinkState", "linked", false];
                };
