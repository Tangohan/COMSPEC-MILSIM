...
if (_allOk) then {
    ...
    private _posTick = missionNamespace getVariable ["COMSPEC_LastPositionSync", -1];
    private _posAgo = [_posTick, "pas encore"] call _fncAgo;
    if (!(_posTick isEqualType 0) || {_posTick < 0}) then {
        private _skip = missionNamespace getVariable ["COMSPEC_PosSkipReason", ""];
        private _hint = switch (_skip) do {
            case "no_terminal": { " — équipez le téléphone ATAK" };
            case "client_state": { " — attendez d’être en mission" };
            case "respawn_grace": { " — reprise après réapparition" };
            case "api_backoff": { " — pause réseau, réessai auto" };
            case "origin_00": { " — déplacez-vous un peu" };
            default { " — envoi auto sous quelques secondes" };
        };
        _posAgo = format ["<t color='#ffe08a'>pas encore%1</t>", _hint];
    };
    ...
    if (!isNull _statusCtrl) then {
        ...
        if (!isNull _host) then {
            (ctrlPosition _host) params ["", "", "_hw", "_hh"];
            private _th = (ctrlTextHeight _statusCtrl) max _hh;
            _statusCtrl ctrlSetPosition [0, 0, _hw, _th];
