...
// Échecs définitifs persistés (chemins Photo Library morts) — survivent aux crash / PreInit.
...

{
    ...
    if (_base isEqualTo "") then {
        private _segs = _filePath splitString "\/";
        _base = toLower (_segs select ((count _segs) - 1));
    };
    if (_key in _seen) then { continue };
    if (_key in _failed) then { continue };
