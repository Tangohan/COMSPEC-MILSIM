/*
    ...
    Params: ["mode_password"|"mode_otp"|"password"|"otp_ask"|"otp_ok"|"steam"|"pair"|"enter"|"logout"]
*/
params [["_action", "", [""]]];
...
private _setLoginMode = {
    params [["_mode", "", [""]]];
    _mode = toLower _mode;
    if (!(_mode in ["", "password", "otp"])) then { _mode = ""; };
    missionNamespace setVariable ["comspec_overwatch_auth_login_mode", _mode, false];
    missionNamespace setVariable ["comspec_overwatch_auth_otp_mode", _mode isEqualTo "otp", false];
    [] call comspec_overwatch_atak_athena_fnc_athena_applyHomeLayout;
    private _grp = [] call comspec_overwatch_atak_athena_fnc_athena_resolveAthenaGroup;
    if (isNull _grp) exitWith {};
    if (_mode isEqualTo "password") then {
        private _p = [_grp, 9793] call comspec_overwatch_atak_athena_fnc_athena_pageCtrl;
        if (!isNull _p) then { ctrlSetFocus _p; };
    };
    if (_mode isEqualTo "otp") then {
        private _o = [_grp, 9794] call comspec_overwatch_atak_athena_fnc_athena_pageCtrl;
        if (!isNull _o) then { ctrlSetFocus _o; };
    };
};
...
switch (_action) do {
    case "mode_password": {
        ["password"] call _setLoginMode;
        ["Saisissez votre mot de passe, puis Se connecter ou Entrée."] call _setHint;
    };
    case "mode_otp": {
        ["otp"] call _setLoginMode;
        ["Recevoir le code, saisissez-le, puis Valider ou Entrée."] call _setHint;
    };
    case "password": {
        ["password"] call _setLoginMode;
    ...
    case "otp_ask": {
        ...
        if (!isNull _otpCtrl) then { _otpCtrl ctrlSetText ""; ctrlSetFocus _otpCtrl; };
        ["Un code vient d’être envoyé. Saisissez-le, puis Valider ou Entrée."] call _setHint;
        [] call comspec_overwatch_connect_fnc_pollAuth;
        [] call comspec_overwatch_atak_athena_fnc_athena_updatePanel;
    };
    case "otp_ok": {
    ...
    case "enter": {
        ...
        // Relecture après ouverture : le bandeau doit passer à Connecté.
        [] call comspec_overwatch_connect_fnc_applyBootstrap;
        private _nowReady = missionNamespace getVariable ["COMSPEC_AthenaReady", false];
        if (!(_nowReady isEqualType true)) then { _nowReady = false; };
        if (_opened || {_nowReady}) then {
            ["Connecté — canal poste ouvert. Position et fiche en cours d’envoi."] call _setHint;
        } else {
            private _err = missionNamespace getVariable ["comspec_overwatch_auth_error", ""];
            if (!(_err isEqualType "")) then { _err = ""; };
            if ((toUpper _err) find "C2_" == 0) then {
                ["Canal poste encore refusé. Déconnectez-vous, puis utilisez un nouveau code Appairer.", true] call _setHint;
            } else {
                ["Canal encore refusé. Réessayez Entrer, ou un nouveau code Appairer.", true] call _setHint;
            };
        };
        [] call comspec_overwatch_atak_athena_fnc_athena_applyHomeLayout;
        [] call comspec_overwatch_atak_athena_fnc_athena_updatePanel;
