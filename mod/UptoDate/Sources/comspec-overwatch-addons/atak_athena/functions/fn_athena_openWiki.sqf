/*
    Ouvre l’app Tutoriel / WIKI dans ATAK Enhanced.
*/
...
[] spawn {
    ...
    ["AtakWiki"] call comspec_overwatch_atak_athena_fnc_athena_openAtakApp;
    uiSleep 0.15;
    if (isNull (uiNamespace getVariable ["COMSPEC_ATAK_Wiki_group", controlNull])) then {
        ["cTab_Android_dlg", [["showMenu", ["AtakWiki", true, ["", -1], createHashMap]]], true, true] call cTab_fnc_setSettings;
        uiSleep 0.12;
    };
    ...
    waitUntil {
        _tries = _tries + 1;
        !isNull (uiNamespace getVariable ["COMSPEC_ATAK_Wiki_group", controlNull])
        || {_tries > 20}
    };
