/*
    Ouvre l’app Sons ATAK dans ATAK Enhanced (tiroir d’applications).
*/
...
[] spawn {
    ...
    ["AtakSound"] call comspec_overwatch_atak_athena_fnc_athena_openAtakApp;
    uiSleep 0.15;
    if (isNull (uiNamespace getVariable ["COMSPEC_ATAK_Sound_group", controlNull])) then {
        ["cTab_Android_dlg", [["showMenu", ["COMSPEC_ATAK_Sound", true, ["", -1], createHashMap]]], true, true] call cTab_fnc_setSettings;
        uiSleep 0.12;
    };
    ...
    waitUntil {
        _tries = _tries + 1;
        !isNull (uiNamespace getVariable ["COMSPEC_ATAK_Sound_group", controlNull])
        || {_tries > 20}
    };
