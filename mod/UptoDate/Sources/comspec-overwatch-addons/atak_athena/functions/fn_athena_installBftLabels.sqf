/*
    ...
    Recolle le calque si un autre addon remplace la fonction après le premier passage.
*/
if (!hasInterface) exitWith {};

if (!isNil "cTab_fnc_updateLists") then {
    private _src = str cTab_fnc_updateLists;
    if ((_src find "athena_relabelBft") < 0) then {
...
if (!isNil "Iceman_fnc_aware_drawbftmarkers") then {
    private _srcAware = str Iceman_fnc_aware_drawbftmarkers;
    if ((_srcAware find "athena_bftUnitLabel") < 0) then {
...
if (isNil "COMSPEC_BftRelabelPulseEH") then {
    COMSPEC_BftRelabelPulseEH = ["cTab_updatePulse", {
        [] call comspec_overwatch_atak_athena_fnc_athena_relabelBft;
    }] call CBA_fnc_addEventHandler;
};

if (isNil "COMSPEC_BftRelabelPFH") then {
    COMSPEC_BftRelabelPFH = [{
