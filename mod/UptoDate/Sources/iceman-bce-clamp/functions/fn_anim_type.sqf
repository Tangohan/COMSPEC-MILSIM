...
if (_instant) exitWith {
  private _p = _End_Point select [0, 4];
  if ((count _p) > 2 && {!isNil {_p select 2}} && {(_p select 2) isEqualType 0} && {(_p select 2) < 0.001}) then { _p set [2, 0.001]; };
  if ((count _p) > 3 && {!isNil {_p select 3}} && {(_p select 3) isEqualType 0} && {(_p select 3) < 0.001}) then { _p set [3, 0.001]; };
