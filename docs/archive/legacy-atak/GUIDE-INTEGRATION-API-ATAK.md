# Guide d'int├®gration API - Nouvelles features ATAK

**Version** : 1.0  
**Date** : 24 juillet 2026  
**Phases impl├®ment├®es** : Phase 1 & 2 (6 features)
...
- `submitter_steam_id` : Steam ID ├®metteur
- `date_from` : Date d├®but (YYYY-MM-DD HH:MM:SS)
- `date_to` : Date fin
...
**R├®ponse** : Rapport cr├®├® avec `id` et `report_number` g├®n├®r├®
...
### 2. Points d'Int├®r├¬t (POI) tactiques
...
- `limit` : Nombre max r├®sultats (d├®faut: 200)
- `offset` : Pagination
  ...
  "pois": [
    {
      ...
      "description": "B├ótiment abandonn├®, activit├® suspecte observ├®e",
      "threat_level": "MEDIUM",
      "status": "ACTIVE",
      "source_type": "VISUAL",
...
```json
{
  ...
  "pos_y": 7890.12,
  "grid_reference": "098765",
  "description": "B├ótiment abandonn├®",
  "threat_level": "MEDIUM",
  "source_type": "VISUAL",
  "source_reliability": "USUALLY_RELIABLE",
  "reported_by_callsign": "RECON-2",
...
#### Mettre ├á jour un POI
```http
PUT /api/atak/poi/{id}
PATCH /api/atak/poi/{id}
```

**Body** : Champs ├á mettre ├á jour (ex: `status`, `certainty`, `threat_level`)
...
- `is_visible` : true | false
- `only_active` : true (filtre zones actuellement actives selon temporalit├®)
- `limit` : Nombre max r├®sultats (d├®faut: 200)
- `offset` : Pagination
...
## Phase 2 : Capacit├®s sp├®cialis├®es
...
- `golden_hour_critical` : true | false (filtre T1 avec golden hour expir├®)
- `only_active` : true
...
```json
{
  ...
  "approach_direction": "Sud-Est recommand├®",
  "remarks": "LZ s├®curis├®e par section Bravo",
  "requested_by_callsign": "MEDIC-1"
}
```

#### D├®tail MEDEVAC avec patients
```http
GET /api/atak/medevac/{id}
```

**R├®ponse** : MEDEVAC + array `patients` avec d├®tails m├®dicaux

#### Mettre ├á jour statut
```http
...
{
  "status": "INBOUND",
  "message": "H├®lico d├®coll├®, ETA 10min"
}
```
...
```json
{
  ...
  "primary_injury": "Blessure par ├®clat thorax",
  "injuries": [
  ...
  "qrfs": [
    {
      ...
      "assigned_qrf_unit": "R├®serve Bravo",
...
```json
{
  ...
  "grid_reference": "890123",
  "threat_type": "AMBUSH",
  "threat_description": "Embuscade convoi, tirs nourris depuis b├ótiments",
  "enemy_strength": "SQUAD",
  "enemy_disposition": "Positions fortifi├®es, HMG visible",
  "requesting_unit": "Convoi Charlie",
  "requesting_callsign": "CHARLIE-6",
  "friendly_strength": 12,
  "friendly_casualties": 2,
  ...
  "terrain_description": "Zone urbaine, rues ├®troites",
...
```json
{
  "qrf_unit": "R├®serve Bravo",
  "qrf_callsign": "BRAVO-QRF"
}
```

#### Mettre ├á jour position QRF
```http
...
**├Ç appeler p├®riodiquement** (ex: toutes les 30s) pendant d├®placement QRF
...
```json
{
  ...
  "update_message": "QRF ├á 1km de la zone, ennemi commence ├á se replier",
  "pos_x": 17000.0,
  "pos_y": 8700.0,
  "is_from_qrf": true
}
...
- `fuel_critical` : true (filtre v├®hicules < 20% fuel)
- `damaged` : true (filtre v├®hicules endommag├®s)
- `limit`, `offset` : Pagination
...
#### Upsert v├®hicule (cr├®er ou mettre ├á jour)
```http
...
**Note** : Upsert par `vehicle_callsign` - cr├®e si n'existe pas, met ├á jour sinon
...
_reportData set ["summary", "Contact ennemi observ├®"];
...
_poiData set ["description", "B├ótiment abandonn├®, activit├® suspecte"];
...
#### Update position v├®hicule
```sqf
// ├Ç ex├®cuter p├®riodiquement (ex: toutes les 5s) pour v├®hicules occup├®s
...
## Int├®gration interface web (JavaScript)

### Exemples d'appels depuis navigateur

#### R├®cup├®rer rapports actifs
```javascript
    ...
    data.vehicles.forEach(vehicle => {
        ...
        if (vehicleMarkers[vehicle.vehicle_callsign]) {
            ...
            const marker = L.marker(pos, {
                ...
                ${vehicle.is_fuel_critical ? 'ÔÜá´©Å CRITIQUE' : ''}<br>
...
- Limites par d├®faut : 100 (rapports), 200 (POI, zones, v├®hicules)
- Utiliser les filtres pour r├®duire la charge r├®seau

### Temps r├®el
- Polling recommand├® : 5-10 secondes pour v├®hicules, 30s pour autres endpoints
- WebSocket planifi├® pour Phase 3 (push temps r├®el)

### Persistance
- Toutes les donn├®es sont multi-tenant (isolation par `tenant_id`)
- Context op├®rationnel (`context_id`) permet plusieurs missions simultan├®es
- Soft delete sur la plupart des ressources (champ `deleted_at`)

### Historique
- Positions v├®hicules sauvegard├®es automatiquement pour replay
- ├ëv├®nements v├®hicules logg├®s automatiquement
- Changements statut MEDEVAC/QRF trac├®s via tables updates
...
Pour questions techniques : Voir code source des repositories pour m├®thodes avanc├®es et options disponibles.
