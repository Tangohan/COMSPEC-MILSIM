...
| `connect/fn_pollAthenaMarkers` | `GetMarkers`, format lignes TSV `M,id,...`, marqueurs locaux | `pollMarkers` puis `localDataRefresh`/`mapOnDraw` | intégré |
...
| `connect/fn_pollOrders` | `GetOrders`, `COMSPEC_Orders` HashMaps | `pollOrders`, page TASK, `taskAction`/`UpdateOrderStatus` | intégré |
| `connect/fn_pollChatMessages` | `GetChatMessages`, inbox | `pollChat`, store messages, page CHAT, `SendChat` | intégré |
