...
2. Le SteamID64 exact est recherché dans ce tenant.
3. En l'absence de compte, un événement `STEAM_ACCOUNT_NOT_FOUND` est conservé et l'API retourne
   `UNLINKED_ARMA_OPERATOR`; aucun compte n'est créé.
...
6. L'empreinte stable d'une anomalie ouverte déduplique les occurrences et incrémente leur compteur.
