...
Chaque appel externe est encadré par un `→` puis un `←` dans le journal Arma (`[COMSPEC Overwatch][DEBUG][Diag]`). Si le prochain plantage tombe dans cet instantané, le dernier `→` sans `←` identifie l’appel. Le contenu brut de la fenêtre de débit (3 premiers éléments) et du retour perte de paquets est aussi écrit.
...
Contrôles automatiques : présence des jalons `→` / `←` et des dumps `pkt raw` / `win count`. En jeu : relancer Arma, lancer le dépannage, ouvrir le `.rpt` : une paire `→` / `←` par cycle. Après un plantage, chercher le dernier `→` sans `←`.
