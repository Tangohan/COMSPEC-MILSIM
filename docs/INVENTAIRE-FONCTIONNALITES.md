...
| **Pilotage communauté** | Canonique : `/formation` (tableau de bord, catalogue, inscriptions, rapports, certificats, journal). Studio : `/formation/studio`. Les **GET** `/admin/training/…` redirigent vers `/formation/…`. L’ancien préfixe `/back-office/ressources/training/…` redirige aussi vers `/formation/…`. |
...
| Formations             | `/formations`, `/formation` (pilotage staff)                 | ✅                   | ✅ pilotage     | `/api/training/*`                 | Formations, Studio / pilotage | Rôles formation + catalogue LMS                        |
