...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  UNIQUE KEY `users_athena_identifier_unique` (`athena_identifier`),
  KEY `tenant_id` (`tenant_id`),
  KEY `role_id` (`role_id`),
...
CREATE TABLE IF NOT EXISTS `units` (
  ...
  `public_founded_on` date DEFAULT NULL,
  `public_custom_date` date DEFAULT NULL,
  `public_custom_date_label` varchar(80) DEFAULT NULL,
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `tenant_id_slug` (`tenant_id`,`slug`),
...
CREATE TABLE IF NOT EXISTS `user_profiles` (
  ...
  `onboarding_persona` varchar(32) DEFAULT NULL,
  `onboarding_steps_json` text DEFAULT NULL,
...
-- Demandes d'élévation (grade / rôle / droits) — machine à états consultable par le demandeur et la cible.
...
CREATE TABLE IF NOT EXISTS `recon_pv_entries` (
  ...
  `mission_text` text,
  `means_text` text,
  `urgency` enum('immediate','deferred') DEFAULT NULL,
  `engagement_frame_text` text,
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
