<div class="max-w-2xl mx-auto px-6 py-12">
    ...
    <form method="post" action="<?= url('admin/organization/categories/store') ?>" class="space-y-4">
        <?= \App\Core\Csrf::field() ?>
        <div>
        ...
        <div>
            ...
            <input type="text" id="slug" name="slug" class="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm" placeholder="Optionnel, généré depuis le nom si vide">
        </div>
        <div>
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="2" class="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm"></textarea>
        </div>
        <div>
            <label for="color" class="block text-sm font-medium text-slate-700">Couleur (hex ou nom)</label>
            <input type="text" id="color" name="color" class="mt-1 block w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm" placeholder="ex: #3B82F6 ou blue">
        </div>
        <div>
