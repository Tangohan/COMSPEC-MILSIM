...
$label = htmlspecialchars((string) ($inviteCode['label'] ?? ''), ENT_QUOTES, 'UTF-8');
...
$assignToOpeningId = isset($inviteCode['assign_to_opening_id']) ? (int) $inviteCode['assign_to_opening_id'] : null;
$defaultSpecialty = htmlspecialchars((string) ($inviteCode['default_specialty'] ?? ''), ENT_QUOTES, 'UTF-8');
?>
...
<div class="space-y-8">
    ...
    <div>
        ...
        <h1 class="mt-4 text-2xl font-bold text-slate-900">Modifier le code d'invitation</h1>
        <p class="mt-2 text-sm text-slate-600">
            Code : <code class="font-mono font-semibold"><?= htmlspecialchars((string) ($inviteCode['code'] ?? ''), ENT_QUOTES, 'UTF-8') ?></code>
        </p>
    </div>

    <!-- Formulaire -->
    <form method="POST" action="<?= htmlspecialchars(url('back-office/recruitments/codes-invitation/' . $codeId . '/modifier'), ENT_QUOTES, 'UTF-8') ?>" class="space-y-8">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8 space-y-6">
            ...
            <div>
                ...
                <input type="datetime-local" 
                       name="expires_at" 
                       id="expires_at" 
                       value="<?= $expiresAt !== null ? date('Y-m-d\TH:i', strtotime((string) $expiresAt)) : '' ?>"
                       class="mt-2 block w-full rounded-lg border-slate-300 bg-white px-4 py-3 text-slate-900 shadow-sm transition focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20">
            </div>
        </div>
                    ...
                    <select name="assign_to_opening_id" 
                        ...
                        <?php foreach ($recruitmentOpenings as $opening): ?>
                            <option value="<?= htmlspecialchars((string) ($opening['id'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
                                    <?= $assignToOpeningId === (int) ($opening['id'] ?? 0) ? 'selected' : '' ?>>
