...
<div class="max-w-2xl mx-auto px-6 py-12">
    ...
    <form method="post" action="<?= url('back-office/groups/store') ?>" class="space-y-4">
        ...
        <?php $unitTypes = $unitTypes ?? config('units.types', []); $defaultType = $defaultType ?? 'group'; ?>
        <div>
            ...
            <select id="struct_type" name="struct_type" class="<?= htmlspecialchars(bo_select_class('mt-1'), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <option value="<?= htmlspecialchars((string) $typeId) ?>" <?= (string) $typeId === (string) $defaultType ? 'selected' : '' ?>><?= htmlspecialchars((string) ($meta['label'] ?? $typeId)) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div>
