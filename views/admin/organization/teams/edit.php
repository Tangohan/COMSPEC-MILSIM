<?php $team = $team ?? null; $parents = $parents ?? []; $users = $users ?? []; if (!$team) { echo '<p>Équipe introuvable.</p>'; return; } $tid = (int) $team['id']; ?>
<div class="max-w-2xl mx-auto px-6 py-12">
    <h1 class="text-2xl font-black text-slate-900 mb-6">Modifier l'équipe</h1>
    <form method="post" action="<?= url('admin/organization/teams/' . $tid . '/update') ?>" class="space-y-4">
        <?= \App\Core\Csrf::field() ?>
        <div>
        ...
        <div class="flex gap-3 pt-4">
            ...
            <a href="<?= url('admin/organization/teams/' . $tid) ?>" class="px-4 py-2 text-slate-600 text-sm hover:underline">Annuler</a>
        </div>
    </form>
</div>
