...
<div class="max-w-xl mx-auto px-6 py-12">
    ...
    <form method="post" action="<?= url('back-office/community') ?>" class="space-y-4">
        ...
        <div>
            <label class="block text-xs font-bold text-slate-700 mb-1">Adresse courte de la page publique</label>
            <input type="text" name="tenant_slug" value="<?= htmlspecialchars((string) ($tenant['slug'] ?? '')) ?>" maxlength="50" required pattern="[a-z0-9]([-a-z0-9]*[a-z0-9])?" class="w-full rounded border border-slate-300 px-3 py-2 text-sm font-mono lowercase" placeholder="mon-unite">
