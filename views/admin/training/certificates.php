...
?>
                <header class="tc-panel p-6 md:p-8">
                    ...
                    <div class="text-slate-600 text-sm space-y-3 max-w-3xl leading-relaxed">
                        ...
                        <div class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-amber-950 text-sm mt-2">
                            <p class="font-semibold">La production des documents est actuellement indisponible.</p>
                            <p class="mt-1"><?= htmlspecialchars(
                                \App\Support\TrainingCertificatePdfEngine::staffUnavailabilityHint()
                                    ?? 'Les attestations restent enregistrées, mais les documents ne pourront pas être produits tant que l’environnement n’est pas corrigé.'
                            ) ?></p>
                        </div>
                        <?php elseif ($trainingCertificatesPendingPdf > 0): ?>
                        <div class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3.5 mt-2">
                            <p class="text-sm text-slate-700"><strong class="font-semibold text-slate-900"><?= (int) $trainingCertificatesPendingPdf ?></strong> attestation<?= $trainingCertificatesPendingPdf > 1 ? 's' : '' ?> <?= $trainingCertificatesPendingPdf > 1 ? 'sont' : 'est' ?> en attente de document.</p>
                            <form method="post" action="<?= htmlspecialchars(training_lms_admin_url('certificates/generer-documents')) ?>" class="mt-3 flex flex-wrap items-center gap-3">
                                <?= \App\Core\Csrf::field() ?>
                                <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-800">Produire les documents en attente</button>
                                <span class="text-xs text-slate-500">Traite en priorité les attestations les plus anciennes ; relancez l’action si toutes ne sont pas traitées en une fois.</span>
                            </form>
                        </div>
                        <?php else: ?>
                        <p class="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-900 text-sm mt-2">Tous les documents sont à jour : aucune attestation en attente.</p>
                        <?php endif; ?>
                        <p class="text-xs text-slate-500">Cette page affiche les attestations les plus récentes.<?php if (!empty($trainingCmdCanEditContent)): ?> · <a href="<?= htmlspecialchars(training_lms_admin_url('certificates/gabarit')) ?>" class="font-semibold text-emerald-700 hover:underline">Personnaliser le gabarit du document</a><?php endif; ?></p>
                    </div>
                </header>
