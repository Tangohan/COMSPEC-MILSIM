<?php
$cpPolicy = is_array($customPagePolicy ?? null) ? $customPagePolicy : [];
$cpCurrentStatus = (string) ($customPage['status'] ?? 'draft');
$cpStatusPolicyKey = ['draft' => null, 'review' => 'review', 'scheduled' => 'publish', 'published' => 'publish', 'archived' => 'archive'];
?>
    ...
    <label class="block">Statut
      <select name="status" class="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2">
        <?php foreach (['draft'=>'Brouillon','review'=>'En revue','scheduled'=>'Programmé','published'=>'Publié','archived'=>'Archivé'] as $k=>$l):
          $policyKey = $cpStatusPolicyKey[$k];
          $available = $k === $cpCurrentStatus || $policyKey === null || !empty($cpPolicy[$policyKey]);
          if (!$available) {
              continue;
          }
        ?>
          <option value="<?= $k ?>" <?= ($cpCurrentStatus===$k?'selected':'') ?>><?= $l ?></option>
        <?php endforeach; ?>
      </select>
      <?php if (empty($cpPolicy['publish']) || empty($cpPolicy['review']) || empty($cpPolicy['archive'])): ?>
      <span class="mt-1 block text-slate-500">Certaines étapes (revue, publication, archivage) demandent des droits que vous n’avez pas — elles n’apparaissent pas ici.</span>
      <?php endif; ?>
    </label>
