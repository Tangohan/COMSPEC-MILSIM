...
<div class="max-w-3xl mx-auto px-6 py-12">
    ...
    <form method="post" action="<?= url('admin/system/cooperation/announcements/save') ?>" class="mt-8 space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div>
            ...
            <input type="text" id="subject" name="subject" maxlength="255" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" value="<?= htmlspecialchars($subjectVal, ENT_QUOTES, 'UTF-8') ?>" placeholder="Sinon un intitulé standard est utilisé.">
        </div>
        <?php endif; ?>
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-1">Si vide, le message peut être publié sur le fil commun du dossier lorsque celui-ci existe.</p>
        </div>
        <div class="flex items-center gap-2">
            ...
            <label for="forum_as_draft" class="text-sm text-slate-700">Enregistrer comme brouillon (à valider avant publication)</label>
        </div>
        <?php endif; ?>
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-1">Limite les doublons pour un même dossier et le même type d’événement. Sans effet sur les autres canaux.</p>
        </div>

        <div class="flex items-center gap-2">
            <input type="checkbox" id="is_active" name="is_active" value="1" class="rounded border-slate-300"<?= $active ? ' checked' : '' ?>>
            <label for="is_active" class="text-sm text-slate-700">Activer ce gabarit</label>
        </div>
