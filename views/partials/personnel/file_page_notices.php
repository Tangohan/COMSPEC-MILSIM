<?php
/**
 * Alertes et actions propres au dossier (absences, restrictions, onglets).
...
<?php $openNotice(); ?>
    <div class="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-violet-200 bg-violet-50/70 px-4 py-2.5">
        <p class="text-xs font-semibold text-violet-900">Vous consultez la fiche telle que les membres la voient.</p>
        <a href="<?= htmlspecialchars($personnelFileBaseUrl . '?view=rh', ENT_QUOTES, 'UTF-8') ?>" class="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-violet-700 hover:text-violet-900">
            <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/></svg>
            Vue commandement
        </a>
    </div>
<?php $closeNotice(); ?>
