...
if ($blocked) {
    ...
    $body = '<p style="margin:0 0 16px;">' . $who . '</p>'
        . email_html_callout(
            '<strong>Signal de détresse repéré</strong> sur le portail de suivi des candidatures de <strong>' . $tn . '</strong>.'
                . '<br><br>Origine du message : <strong>' . $side . '</strong>.'
                . '<br>Motif : <strong>' . $cat . '</strong>.'
                . '<br><br>Aucun blocage n’a été appliqué : le message a été transmis normalement au fil de la candidature. Un suivi humain rapide est recommandé.',
            'warning'
        )
        . '<p style="margin:16px 0 8px;font-size:13px;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:0.06em;">Extrait (partiellement masqué)</p>'
        . '<div style="padding:12px 14px;background-color:#f8fafc;border-radius:10px;border:1px solid #e2e8f0;font-size:14px;color:#475569;">' . $prev . '</div>';
...
$html = email_html_layout(
    ($blocked ? 'Alerte modération' : 'Vigilance — signal de détresse') . ' — dossier recrutement #' . $eid,
    $blocked ? 'Alerte modération — recrutement' : 'Vigilance — recrutement',
    $body,
    ['accent' => 'amber']
);

$text = ($blocked ? 'Alerte modération' : 'Vigilance — signal de détresse') . " — « {$tenantName} » — dossier n°{$eid}\n"
    . "Rôle destinataire : {$recipientAudience}\n"
    . "Origine : {$sourceSideLabel}\n"
    . "Motif : {$categoryLabel}\n"
    . "Extrait : {$maskedPreview}\n"
    . ($blocked ? '' : "Aucun blocage appliqué — le message a été transmis normalement. Suivi humain recommandé.\n");
if ($recipientAudience !== 'candidate' && $manageUrl !== '') {
