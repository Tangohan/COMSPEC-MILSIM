...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .panel { position:relative; color:var(--ink); background:rgba(242,240,233,.96); box-shadow:0 32px 90px rgba(0,0,0,.38); }
        ...
        .copy p { margin:0; color:var(--muted); font-size:14px; font-weight:500; line-height:1.72; }
        ...
        @media (max-width:900px) { .layout { grid-template-columns:1fr; align-items:start; } .intro { max-width:700px; } .panel { max-width:650px; } h1 { font-size:clamp(3.4rem,14vw,7rem); } }
        @media (max-width:560px) { .topbar { height:64px; } .classification { display:none; } .layout { min-height:calc(100svh - 64px); padding-top:42px; gap:38px; } h1 { font-size:clamp(3rem,15vw,5rem); } .lede { margin-top:26px; } .meta { margin-top:30px; gap:20px; flex-direction:column; } .panel__head,.copy,.panel__foot { padding-left:22px; padding-right:22px; } .reopen { margin-left:22px; margin-right:22px; } .panel__foot { align-items:flex-start; flex-direction:column; } .cta { width:100%; justify-content:space-between; } }
