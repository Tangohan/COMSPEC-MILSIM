...
<body class="bg-slate-50 text-slate-900 selection:bg-slate-900 selection:text-white overflow-x-hidden">
        ...
        <div class="max-w-5xl mx-auto px-6 sm:px-8 h-16 flex items-center justify-between relative text-slate-900 uppercase">
            ...
            <div class="flex flex-1 justify-end items-center gap-4 sm:gap-6">
                ...
                <div class="hidden sm:flex flex-col items-end leading-none">
                    <span class="text-[7px] font-black tracking-[0.28em] text-slate-400"><?= htmlspecialchars(__('errors.403_clock'), ENT_QUOTES, 'UTF-8') ?></span>
                    <span id="err403-clock" class="text-[10px] font-mono font-semibold tracking-wide text-slate-700 tabular-nums">--:--:--</span>
                </div>
                <div class="flex items-center gap-2 border-l border-slate-200 pl-4 sm:pl-6">
                    <span class="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(245,158,11,0.5)]" aria-hidden="true"></span>
                    <span class="text-[8px] font-black tracking-[0.2em] text-slate-400"><?= htmlspecialchars(__('errors.403_error_active'), ENT_QUOTES, 'UTF-8') ?></span>
                </div>
            </div>
        </div>
    </header>
    ...
    <main>
        <section class="relative min-h-[78vh] w-full overflow-hidden bg-black tactical-grid-dark" aria-labelledby="err403-heading">
                ...
                <div class="flex justify-between items-start gap-8">
                    <div class="flex flex-col gap-2">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[9px] font-black tracking-[0.4em] text-white uppercase"><?= htmlspecialchars(__('errors.403_alert'), ENT_QUOTES, 'UTF-8') ?></span>
                        </div>
                        <span class="text-[7px] font-bold text-white/30 tracking-[0.3em] uppercase max-w-md"><?= htmlspecialchars(__('errors.403_alert_sub'), ENT_QUOTES, 'UTF-8') ?></span>
                    </div>
                    <div class="text-right flex flex-col items-end">
                        <span id="err403-timestamp" class="text-[9px] font-mono text-white/40 tracking-widest uppercase mb-1"></span>
                        <span class="text-[7px] font-bold text-white/20 tracking-[0.5em] uppercase"><?= htmlspecialchars(__('errors.403_sector'), ENT_QUOTES, 'UTF-8') ?></span>
                    </div>
                </div>
                ...
                <div class="max-w-3xl mt-16 md:mt-0">
                    ...
                    <div class="mt-8 flex flex-wrap gap-3">
                        <a href="<?= htmlspecialchars($base) ?>/" class="inline-flex items-center justify-center px-5 py-3 rounded-2xl bg-white text-slate-950 text-[10px] font-black uppercase tracking-[0.22em] hover:bg-emerald-50 transition-colors">
                            <?= htmlspecialchars(__('errors.403_home'), ENT_QUOTES, 'UTF-8') ?>
                        </a>
                        <?php if ($loggedIn): ?>
                        ...
                        <button type="button" onclick="history.back()" class="inline-flex items-center justify-center px-5 py-3 rounded-2xl border border-white/10 text-white/80 text-[10px] font-black uppercase tracking-[0.22em] hover:bg-white/[0.06] transition-colors">
                            <?= htmlspecialchars(__('errors.403_back'), ENT_QUOTES, 'UTF-8') ?>
                        </button>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        (function () {
            ...
            function tickClock() {
                var el = document.getElementById('err403-clock');
                if (!el) return;
                var d = new Date();
                el.textContent = pad(d.getHours()) + ':' + pad(d.getMinutes()) + ':' + pad(d.getSeconds());
            }
            function tickTs() {
                var el = document.getElementById('err403-timestamp');
                if (!el) return;
