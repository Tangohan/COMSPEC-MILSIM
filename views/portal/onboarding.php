            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                ...
                <ol class="mt-6 space-y-4 text-sm text-slate-700">
                    <?php foreach (($activePersona['steps'] ?? []) as $idx => $personaStep): ?>
                        <li class="flex gap-3">
                            <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-800"><?= (int) $idx + 1 ?></span>
                            <span><a class="font-semibold text-emerald-800 underline-offset-2 hover:underline" href="<?= htmlspecialchars((string) $personaStep['href'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars((string) $personaStep['label'], ENT_QUOTES, 'UTF-8') ?></a> — <?= htmlspecialchars((string) $personaStep['description'], ENT_QUOTES, 'UTF-8') ?></span>
                        </li>
                    <?php endforeach; ?>
                </ol>
