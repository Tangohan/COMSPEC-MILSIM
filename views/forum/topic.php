...
?>
<main class="w-full max-w-6xl mx-auto px-4 sm:px-6 py-10">
  <!-- Fil d'Ariane -->
  <nav class="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.25em] text-neutral-800 mb-8 flex-wrap anim-up" style="animation-delay:0ms">
    <a href="<?= $baseUrl ?>/forum" class="hover:text-neutral-500 transition-colors">Forum</a>
    <span class="text-neutral-800">›</span>
    <a href="<?= $baseUrl ?>/forum/category/<?= htmlspecialchars($topic['category_slug'] ?? '') ?>" class="hover:text-neutral-500 transition-colors"><?= htmlspecialchars($topic['category_name'] ?? '') ?></a>
    <span class="text-neutral-800">›</span>
    <span class="text-neutral-600 truncate max-w-[200px] normal-case tracking-normal font-bold"><?= htmlspecialchars($topic['title']) ?></span>
  </nav>
  ...
  <div class="mb-10 anim-up" style="animation-delay:40ms">
    <div class="flex items-center gap-3 mb-3">
      ...
      <span class="text-[8px] font-black uppercase tracking-[0.5em] text-orange-500/60"><?= htmlspecialchars($topic['category_name'] ?? '') ?></span>
    </div>
    <div class="flex items-start justify-between gap-6 flex-wrap">
      <div class="flex-1 min-w-0">
        <h1 class="text-3xl md:text-4xl font-black italic uppercase tracking-tighter text-white leading-tight mb-3">
          <?= htmlspecialchars($topic['title']) ?>
        </h1>
        <div class="flex items-center gap-2 flex-wrap">
          <span class="text-[9px] text-neutral-700 font-bold">
            Par <span class="text-neutral-500"><?= htmlspecialchars($topic['author_name'] ?? '') ?></span>
      ...
      <div class="shrink-0 flex gap-2 flex-wrap items-start">
        <button type="button" id="topic-sub-btn" data-topic-id="<?= $topicId ?>" data-subscribed="<?= !empty($isSubscribed) ? '1' : '0' ?>" class="flex items-center gap-2 px-3 py-1.5 text-[8px] font-black uppercase tracking-widest border transition-colors <?= !empty($isSubscribed) ? 'bg-indigo-500/15 border-indigo-500/35 text-indigo-400' : 'bg-white/[0.03] border-white/10 text-neutral-500 hover:text-indigo-400' ?>">
        ...
        <?php if (!empty($isModo)): ?>
          <div class="flex gap-1.5 flex-wrap" id="modo-actions">
            ...
            <button type="button" class="topic-modo-btn px-3 py-1.5 text-[8px] font-black uppercase tracking-widest bg-indigo-500/10 border border-indigo-500/25 text-indigo-400 hover:bg-indigo-500/20 transition-colors" data-action="<?= !empty($topic['is_pinned']) ? 'unpin_topic' : 'pin_topic' ?>"><?= !empty($topic['is_pinned']) ? 'Désépingler' : 'Épingler' ?></button>
  ...
  <?php if (!empty($topic['is_locked'])): ?>
    <div class="border border-amber-500/30 bg-amber-500/5 p-4 mb-6 text-amber-200/90 text-sm">Ce sujet est verrouillé — les nouvelles réponses ne sont pas acceptées.</div>
  <?php endif; ?>

  <?php $flashSuccess = \App\Core\Session::getFlash('success'); $flashError = \App\Core\Session::getFlash('error'); ?>
  <?php if ($flashSuccess): ?><p class="mb-4 text-sm text-emerald-400"><?= htmlspecialchars($flashSuccess) ?></p><?php endif; ?>
  <?php if ($flashError): ?><p class="mb-4 text-sm text-rose-400"><?= htmlspecialchars($flashError) ?></p><?php endif; ?>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="flex flex-wrap items-center gap-2 mb-4">
      ...
      <?php for ($i = 1; $i <= min($totalPages, 8); $i++): ?>
        <a href="<?= $baseUrl ?>/forum/topic/<?= $topicId ?>?page=<?= $i ?>" class="min-w-[2rem] py-1.5 text-center border text-[10px] font-bold <?= $i === $page ? 'bg-orange-500 border-orange-500 text-black' : 'border-white/10 text-neutral-500 hover:text-white' ?> transition"><?= $i ?></a>
      <?php endfor; ?>
    </div>
  <?php endif; ?>

  <!-- Liste des messages -->
  <div id="posts-list" class="flex flex-col gap-px bg-white/[0.025] mb-10">
    <?php
    $postBodiesForJs = [];
    if (!empty($posts)) {
        foreach ($posts as $p) {
    ...
    ?>
    <?php if (empty($posts)): ?>
      <p class="py-8 text-center text-neutral-500">Aucun message dans ce sujet.</p>
    <?php else: ?>
      <?php foreach ($posts as $post): ?>
        ...
        <div id="post-<?= (int) $post['id'] ?>" class="group post-card <?= $roleClass ?>" data-post-id="<?= (int) $post['id'] ?>">
          <div class="flex flex-col md:flex-row">
            <div class="author-card shrink-0 md:w-44 bg-[#08080a] border-b md:border-b-0 md:border-r border-white/10 p-5 flex md:flex-col items-center md:items-start gap-5 relative overflow-hidden">
              ...
              <div class="relative shrink-0 z-10">
                <div class="absolute -top-1 -left-1 w-3 h-3 border-t border-l <?= $accentIsOrange ? 'border-orange-500' : 'border-red-500' ?> z-20 pointer-events-none"></div>
                <div class="absolute -bottom-1 -right-1 w-3 h-3 border-b border-r <?= $accentIsOrange ? 'border-orange-500' : 'border-red-500' ?> z-20 pointer-events-none"></div>
                <div class="relative w-14 h-14 md:w-20 md:h-20 overflow-hidden border border-white/10 group-hover:border-white/25 transition-colors duration-300 bg-white/5 flex items-center justify-center">
                  <?php if ($avatarUrl): ?>
                  ...
                  <div class="absolute inset-0 bg-gradient-to-t <?= $accentIsOrange ? 'from-orange-900/40' : 'from-red-900/40' ?> to-transparent opacity-60 group-hover:opacity-80 transition-opacity duration-300"></div>
                  <?php else: ?>
                  <span class="text-xl md:text-2xl font-black italic text-white"><?= $initial ?></span>
                  <?php endif; ?>
                </div>
              </div>
              <div class="flex-1 md:flex-none min-w-0 w-full space-y-2.5 z-10 md:mt-3">
                ...
                <div class="relative inline-block">
                  <span class="rank-badge-glow absolute inset-0 rounded opacity-30 blur-sm <?= $accentIsOrange ? 'bg-orange-500' : 'bg-red-500' ?>"></span>
                  <span class="relative block text-[7px] font-black uppercase tracking-[0.18em] px-2 py-1 <?= $accentIsOrange ? 'bg-orange-500/10 border-orange-500/40 text-orange-400' : 'bg-red-500/10 border-red-500/40 text-red-400' ?> border"><?= htmlspecialchars($roleName) ?></span>
                </div>
                <?php endif; ?>
                ...
                <div class="hidden md:block pt-2.5 border-t border-white/[0.05] space-y-1.5">
                  ...
                  <div class="flex items-center justify-between gap-2">
                    <span class="text-[7px] font-bold text-neutral-700 uppercase tracking-widest">Grade</span>
                    <span class="text-[8px] font-black text-white"><?= htmlspecialchars($authorGradeName) ?></span>
                  </div>
                  <?php endif; ?>
                  <?php if ($authorGradeNato): ?>
                  <div class="flex items-center justify-between gap-2">
                    <span class="text-[7px] font-bold text-neutral-700 uppercase tracking-widest">Grade OTAN</span>
                    <span class="text-[8px] font-black text-neutral-400"><?= htmlspecialchars($authorGradeNato) ?></span>
                  </div>
                  <?php endif; ?>
                  <?php if ($authorPrimaryRole): ?>
                  <div class="flex items-center justify-between gap-2">
                    <span class="text-[7px] font-bold text-neutral-700 uppercase tracking-widest">Rôle</span>
                    <span class="text-[8px] font-black text-white truncate max-w-[100px]" title="<?= htmlspecialchars($authorPrimaryRole) ?>"><?= htmlspecialchars($authorPrimaryRole) ?></span>
                  </div>
                  <?php endif; ?>
...
<span class="text-[8px] font-black text-white truncate max-w-[100px]" title="<?= htmlspecialchars($authorUnitDisplay) ?>"><?= htmlspecialchars($authorUnitDisplay) ?></span>
                  </div>
                  <?php endif; ?>
                  <?php if ($authorUnitDepth !== null): ?>
                  <div class="flex items-center justify-between gap-2">
                    <span class="text-[7px] font-bold text-neutral-700 uppercase tracking-widest">Niv. ORBAT</span>
                    <span class="text-[8px] font-black <?= $accentIsOrange ? 'text-orange-400' : 'text-red-400' ?>"><?= $authorUnitDepth ?></span>
                  </div>
                  <?php endif; ?>
                  <?php if ($authorAwards): ?>
                  <div class="pt-1">
                ...
                <div class="hidden md:block pt-2.5 border-t border-white/[0.05] space-y-1.5">
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <span class="text-[8px] font-black tabular-nums <?= $accentIsOrange ? 'text-orange-400' : 'text-red-400' ?>">LVL_<?= $level ?></span>
                  </div>
                  <div class="flex items-center justify-between gap-2">
                    <span class="text-[7px] font-bold text-neutral-700 uppercase tracking-widest">Contrib.</span>
                    <span class="text-[8px] font-black text-white tabular-nums"><?= str_pad((string) $postCount, 3, '0', STR_PAD_LEFT) ?></span>
                  </div>
            ...
            <div class="post-role-frame flex-1 min-w-0 flex flex-col bg-[#070708]">
              <div class="flex items-center justify-between px-5 py-2.5 border-b border-white/[0.04] bg-black/20">
                <div class="flex items-center gap-3 flex-wrap">
                  <span class="text-[8px] font-black uppercase tracking-[0.2em] text-neutral-700">#<?= (int) $post['id'] ?></span>
                  <span class="text-[9px] text-neutral-600 font-bold"><?= function_exists('forum_time_ago') ? forum_time_ago($post['created_at'] ?? '') : date('d/m/Y H:i', strtotime($post['created_at'] ?? 'now')) ?></span>
                  <?php if ($postIsHidden && !empty($isModo)): ?><span class="text-rose-400 text-[8px]">· Masqué</span><?php endif; ?>
                </div>
                <a href="#post-<?= (int) $post['id'] ?>" class="text-[10px] text-neutral-800 hover:text-neutral-600 transition-colors font-bold select-none">¶</a>
              </div>
              <div class="flex-1 px-5 py-5">
                <div class="post-content text-sm text-neutral-200 leading-relaxed rounded-xl border border-white/5 bg-gradient-to-br from-white/[0.03] to-transparent p-4 md:p-5 ring-1 ring-orange-500/20 shadow-[0_0_30px_rgba(249,115,22,0.06)]">
                  <?= function_exists('forum_render_content') ? forum_render_content($post['body'] ?? '') : nl2br(htmlspecialchars($post['body'] ?? '')) ?>
                </div>
              </div>
              <div class="border-t border-white/[0.04] px-5 py-3 flex items-center justify-between gap-4 flex-wrap bg-black/15">
                <div class="flex items-center gap-1 flex-wrap"></div>
                <div class="flex items-center gap-4 ml-auto flex-wrap">
                  <?php if ($userId): ?><button type="button" class="post-quote-btn text-[8px] font-black uppercase tracking-widest text-neutral-600 hover:text-indigo-400 transition-colors flex items-center gap-1.5" data-post-id="<?= (int) $post['id'] ?>" data-author="<?= htmlspecialchars($post['author_name'] ?? '') ?>"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"></path></svg> Citer</button><?php endif; ?>
  ...
  <?php if (!empty($canReply)): ?>
    <div class="bg-[#070708] border border-white/[0.05] p-6 md:p-7 anim-up" style="animation-delay:200ms" id="reply-form">
      <div class="flex items-center gap-3 mb-5 pb-4 border-b border-white/[0.04]">
        <div class="w-9 h-9 bg-orange-500 text-black font-black italic flex items-center justify-center text-sm select-none">
          <?= mb_strtoupper(mb_substr(\App\Core\Session::get('display_name') ?? 'U', 0, 1)) ?>
        </div>
        <div>
          <p class="text-[8px] font-black uppercase tracking-[0.3em] text-orange-500">Votre réponse</p>
          <p class="text-xs font-black text-white"><?= htmlspecialchars(\App\Core\Session::get('display_name') ?? '') ?></p>
        </div>
      </div>
      <form method="post" action="<?= $baseUrl ?>/forum/topic/<?= $topicId ?>/reply">
        <?= \App\Core\Csrf::field() ?>
        <textarea name="body" id="reply-content" rows="6" maxlength="10000" class="w-full bg-[#0d0d10] border border-white/[0.07] px-5 py-4 text-sm text-white placeholder-neutral-700 focus:border-orange-500/25 resize-y transition-colors font-mono leading-relaxed" placeholder="Écrivez votre réponse… Markdown supporté : **gras**, *italique*, `code`, &gt; citation" required></textarea>
        <p class="mt-2 text-[8px] uppercase tracking-[0.2em] text-neutral-700 font-black">Raccourcis: Ctrl/Cmd+B, Ctrl/Cmd+I</p>
        <div class="flex items-center justify-between mt-4">
          <span class="text-[9px] text-neutral-700 font-bold tabular-nums" id="reply-char-count">0 / 10 000</span>
          <button type="submit" class="inline-flex items-center gap-3 px-7 py-3 bg-orange-500 hover:bg-orange-400 text-black font-black uppercase text-[9px] tracking-[0.2em] transition-colors">
  ...
  <?php elseif (empty($topic['is_locked']) && empty($topic['is_archived'])): ?>
    <p class="mt-10 pt-8 border-t border-white/5 text-neutral-500 text-sm">Vous ne pouvez pas répondre à ce sujet.</p>
  <?php else: ?>
    <p class="mt-10 pt-8 border-t border-white/5 text-neutral-500 text-sm">Ce sujet est verrouillé ou archivé. Les nouvelles réponses ne sont pas acceptées.</p>
  <?php endif; ?>
</main>

<?php if (!empty($moderationTutorialHtml)): ?>
<div id="forum-tutorial-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/80 p-4" aria-hidden="true" style="display: none;">
  <div class="bg-[#0a0a0c] border border-white/10 max-w-2xl w-full max-h-[80vh] overflow-auto p-6">
    <div class="prose prose-invert max-w-none text-sm"><?= $moderationTutorialHtml ?></div>
    <button type="button" class="mt-4 border border-white/10 px-4 py-2 text-[10px] font-bold uppercase text-neutral-400 hover:text-white" data-close-modal="forum-tutorial-modal">Fermer</button>
  </div>
</div>
<?php endif; ?>

<div id="report-modal" class="hidden fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
  <div class="bg-[#0d0d10] border border-white/[0.08] p-6 max-w-md w-full">
    <div class="flex items-center gap-3 mb-5 pb-4 border-b border-white/[0.05]">
      ...
      <h3 class="text-sm font-black uppercase tracking-widest text-white">Signaler ce contenu</h3>
    </div>
    <form id="report-form">
      <input type="hidden" id="report-target-type" value="post">
      <input type="hidden" id="report-target-id" value="">
      <p class="text-[8px] font-black uppercase tracking-widest text-neutral-600 mb-2">Motif</p>
      <select id="report-reason" class="w-full bg-[#111115] border border-white/[0.08] px-3 py-2.5 text-sm text-white mb-4 focus:border-rose-500/30 focus:outline-none transition-colors">
      ...
      <p class="text-[8px] font-black uppercase tracking-widest text-neutral-600 mb-2">Détails (optionnel)</p>
      <textarea id="report-details" rows="3" placeholder="Précisez si nécessaire…" class="w-full bg-[#111115] border border-white/[0.08] px-3 py-2.5 text-sm text-white placeholder-neutral-700 focus:border-rose-500/25 focus:outline-none mb-5 transition-colors resize-none"></textarea>
      <div class="flex gap-3">
        <button type="submit" class="flex-1 py-2.5 bg-rose-600 hover:bg-rose-500 text-white text-[8px] font-black uppercase tracking-widest transition-colors">Envoyer</button>
        <button type="button" class="flex-1 py-2.5 bg-white/5 hover:bg-white/10 text-neutral-500 text-[8px] font-black uppercase tracking-widest transition-colors" data-close-modal="report-modal">Annuler</button>
      </div>
    </form>
  </div>
</div>

<div id="edit-post-modal" class="hidden fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
  <div class="bg-[#0d0d10] border border-white/[0.08] p-6 max-w-2xl w-full max-h-[90vh] flex flex-col">
    <div class="flex items-center gap-3 mb-5 pb-4 border-b border-white/[0.05]">
      <svg class="w-4 h-4 text-amber-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
      <h3 class="text-sm font-black uppercase tracking-widest text-white">Modifier le message</h3>
    </div>
    <textarea id="edit-post-content" rows="10" maxlength="10000" class="w-full bg-[#111115] border border-white/[0.08] px-4 py-3 text-sm text-white placeholder-neutral-700 focus:border-amber-500/30 focus:outline-none resize-y mb-4 font-mono leading-relaxed"></textarea>
    <p class="text-[8px] text-neutral-600 mb-4"><span id="edit-post-char-count">0</span> / 10 000</p>
    <div class="flex gap-3">
      <button type="button" id="edit-post-submit" class="flex-1 py-2.5 bg-amber-500 hover:bg-amber-400 text-black text-[8px] font-black uppercase tracking-widest transition-colors">Enregistrer</button>
      <button type="button" id="edit-post-cancel" class="flex-1 py-2.5 bg-white/5 hover:bg-white/10 text-neutral-500 text-[8px] font-black uppercase tracking-widest transition-colors">Annuler</button>
    </div>
  </div>
</div>

<div id="forum-toast" class="hidden fixed bottom-6 right-6 z-50 px-5 py-3 text-[10px] font-black uppercase tracking-widest shadow-2xl transition-all bg-[#0a0a0c] border border-white/10 text-white"></div>
    ...
    subBtn.addEventListener('click', function() {
      ...
      fetch(baseUrl + '/api/forum', {
        ...
        if (d.success) {
          ...
          subBtn.className = 'flex items-center gap-2 px-3 py-1.5 text-[8px] font-black uppercase tracking-widest border transition-colors ' + (subscribed ? 'bg-white/[0.03] border-white/10 text-neutral-500 hover:text-indigo-400' : 'bg-indigo-500/15 border-indigo-500/35 text-indigo-400');
