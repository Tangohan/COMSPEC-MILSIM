...
<div class="flex min-h-screen flex-col">
    <div class="border-b border-red-500/10 bg-red-950/30 px-5 py-6 md:px-7 md:py-6">
        <div class="mx-auto flex max-w-3xl items-center gap-4 md:gap-5">
            <div class="relative shrink-0">
                <div class="absolute inset-0 rounded-2xl bg-red-500 opacity-20 blur-xl" aria-hidden="true"></div>
                <div class="leave-warn-pulse relative flex h-12 w-12 items-center justify-center rounded-2xl border-2 border-red-500/70 bg-[#0a0a0c]" aria-hidden="true">
            ...
            <div class="min-w-0">
                <h1 class="text-xl font-black uppercase italic leading-none tracking-tighter text-white">Tu quittes <?= $brandHeadlineSafe ?></h1>
                <p class="mt-1.5 text-[11px] font-bold leading-relaxed text-red-400/80">Ce lien ouvre un site externe, hors du portail — reste prudent·e.</p>
            </div>
        </div>
    </div>

    <div class="border-b border-white/[0.06] bg-slate-900/40 px-5 py-2.5 md:px-7">
        <div class="mx-auto flex max-w-3xl items-start gap-2.5 rounded-lg border border-emerald-500/20 bg-emerald-950/25 px-3 py-2 text-[11px] leading-snug text-emerald-100/90">
            <span class="mt-0.5 shrink-0 font-black tracking-tight text-emerald-400" aria-hidden="true"><?= $brandHeadlineSafe ?></span>
            <p class="text-emerald-100/85"><span class="font-semibold text-white">Rappel portail.</span> Tu es connecté·e à l’espace sécurisé <?= $subtitleSafe ?> (<?= $forumName ?>). Ce message s’affiche pour te laisser le choix avant d’ouvrir une page tiers.</p>
        </div>
    </div>
    ...
    <div class="flex flex-1 flex-col items-center px-4 py-10 sm:py-12">
        <div class="w-full max-w-lg">
            <p class="mb-6 text-center text-sm leading-relaxed text-slate-400">
                Bonjour <strong class="font-semibold text-white"><?= htmlspecialchars($leaveUserDisplayName, ENT_QUOTES | ENT_HTML5, 'UTF-8') ?></strong>
            ...
            <div class="rounded-2xl border border-white/10 bg-slate-900/60 p-6 shadow-2xl shadow-black/40 backdrop-blur-sm sm:p-8">
                <div class="mb-6 rounded-xl border border-white/10 bg-black/30 p-4">
                    ...
                    <p class="break-words text-sm font-bold text-white"><?= htmlspecialchars($leaveDomain, ENT_QUOTES | ENT_HTML5, 'UTF-8') ?></p>
                    <p class="mt-2 break-all font-mono text-[11px] leading-snug text-slate-500"><?= htmlspecialchars($leaveTargetUrl, ENT_QUOTES | ENT_HTML5, 'UTF-8') ?></p>
                    <?php if (!empty($leaveIsHttps)): ?>
                    <p class="mt-2 text-[11px] font-medium text-emerald-400/90">La connexion vers ce site est chiffrée (recommandé).</p>
                    <?php endif; ?>
                </div>
                ...
                <ul class="mb-8 space-y-4">
                    ...
                    <li class="flex gap-3 rounded-xl border border-white/5 bg-white/[0.03] p-3.5">
                        <span class="shrink-0 text-lg leading-none" aria-hidden="true"><?= htmlspecialchars($tip['icon'], ENT_QUOTES, 'UTF-8') ?></span>
                        <div class="min-w-0">
                            <p class="text-sm font-bold text-slate-100"><?= htmlspecialchars($tip['title'], ENT_QUOTES, 'UTF-8') ?></p>
                            <p class="mt-1 text-xs leading-relaxed text-slate-400"><?= htmlspecialchars($tip['body'], ENT_QUOTES, 'UTF-8') ?></p>
                        </div>
                    </li>
                    <?php endforeach; ?>
                </ul>
                ...
                <label class="mb-6 flex cursor-pointer select-none items-start gap-3 group">
                    <input type="checkbox" id="leave-accept" class="mt-1 h-4 w-4 rounded border-slate-600 bg-slate-800 text-red-600 focus:ring-red-500 focus:ring-offset-0">
                    <span class="text-sm leading-snug text-slate-300 group-hover:text-slate-200">
                ...
                <div class="mb-6">
                    <div class="mb-2 flex flex-wrap justify-between gap-2 text-xs text-slate-500">
                        <span>Bouton actif dans <strong id="leave-countdown" class="tabular-nums text-slate-200"><?= (int) $countdown ?></strong> s</span>
                        <span class="flex flex-wrap gap-x-3 gap-y-1">
                            <a href="<?= htmlspecialchars($baseUrl, ENT_QUOTES) ?>/dashboard" class="font-medium text-emerald-400 hover:text-emerald-300">Tableau de bord</a>
                            <a href="<?= htmlspecialchars($baseUrl, ENT_QUOTES) ?>/forum" class="font-medium text-emerald-400 hover:text-emerald-300">Forum</a>
                        </span>
                    </div>
                ...
                <div class="flex flex-col gap-3">
                    <a href="<?= htmlspecialchars($baseUrl, ENT_QUOTES) ?>/forum" class="inline-flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-slate-200 transition-colors hover:bg-white/10">
                        Annuler — rester sur le portail
                    </a>
                    <button type="button" id="leave-continue" disabled class="inline-flex cursor-not-allowed items-center justify-center rounded-xl bg-slate-700 px-4 py-3 text-sm font-semibold text-slate-400 transition-colors">
                        Ouvrir le site externe
                    </button>
                </div>
            </div>

            <p class="mt-6 text-center text-[10px] uppercase tracking-widest text-slate-600"><?= $subtitleSafe ?> · sécurité des liens</p>
        </div>
    </div>
</div>
...
(function() {
    ...
    function setButtonState() {
        ...
        if (ok) {
            ...
            btn.classList.add('bg-red-700', 'hover:bg-red-800', 'text-white', 'cursor-pointer', 'shadow-lg', 'shadow-red-900/30');
        } else {
            btn.classList.add('bg-slate-700', 'text-slate-400', 'cursor-not-allowed');
            btn.classList.remove('bg-red-700', 'hover:bg-red-800', 'text-white', 'cursor-pointer', 'shadow-lg', 'shadow-red-900/30');
        }
    }
