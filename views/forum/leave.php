...
<div class="max-w-2xl mx-auto px-4 py-10">
    <div class="flex items-start gap-4 mb-8">
        <div class="flex-shrink-0 w-12 h-12 bg-red-600 rounded-lg flex items-center justify-center text-white text-2xl font-black">!</div>
        <div>
            <h1 class="text-2xl font-black italic uppercase tracking-tight text-slate-900 mb-2">Tu quittes <?= $forumName ?></h1>
            <p class="text-sm text-red-700">Ce lien mène vers un site externe non contrôlé par nous.</p>
        </div>
    </div>

    <p class="text-xs uppercase tracking-widest text-slate-500 mb-2">Destination</p>
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm p-4 mb-8">
        <div class="flex items-start gap-3">
            <div class="w-10 h-10 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-700 text-sm">🔗</div>
            <div class="min-w-0 flex-1">
                <p class="font-bold text-slate-900 uppercase tracking-wide truncate"><?= htmlspecialchars($leaveDomain, ENT_QUOTES | ENT_HTML5, 'UTF-8') ?></p>
                <p class="text-xs text-slate-500 break-all mt-1"><?= htmlspecialchars($leaveTargetUrl, ENT_QUOTES | ENT_HTML5, 'UTF-8') ?></p>
            </div>
    ...
    <div class="mb-8">
        ...
        <ul class="space-y-4 text-sm text-slate-600 border-l-2 border-slate-200 pl-4">
    ...
    <div class="mb-6">
        <div class="flex justify-between text-xs text-neutral-500 mb-2">
            <span>Disponible dans <strong id="leave-countdown" class="text-slate-900"><?= (int) $countdown ?></strong>s</span>
            <a href="<?= htmlspecialchars($baseUrl, ENT_QUOTES) ?>/forum" class="text-emerald-700 hover:text-emerald-600">ou retourne au forum</a>
        </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3">
        <a href="<?= htmlspecialchars($baseUrl, ENT_QUOTES) ?>/forum" class="inline-flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-slate-100 border border-slate-200 text-slate-800 text-sm font-semibold hover:bg-slate-200">
            ← Retour au forum
        </a>
    ...
    <p class="text-center text-xs text-neutral-500 mt-8">
        Bonjour <strong class="text-slate-800"><?= htmlspecialchars($leaveUserDisplayName, ENT_QUOTES | ENT_HTML5, 'UTF-8') ?></strong> — reste vigilant·e en dehors de la salle.
    </p>
