...
$canSetMissionPriority = !empty($canSetMissionPriority);
$missionPriorityLevelDefault = trim((string) ($missionPriorityLevelDefault ?? ''));
$missionPriorityRoleDefault = trim((string) ($missionPriorityRoleDefault ?? ''));
$mandatoryReadDefault = !empty($mandatoryReadDefault);
$mandatoryReadDueAtDefault = trim((string) ($mandatoryReadDueAtDefault ?? ''));
  ...
  <div class="bg-white border border-slate-200 shadow-lg relative overflow-hidden rounded-xl">
    ...
    <div class="flex flex-col lg:flex-row gap-0 lg:gap-6 p-6 sm:p-8">
      ...
      <form id="new-topic-form" class="flex-1 min-w-0 space-y-6">
        ...
        <?php if ($canSetMissionPriority): ?>
        <div class="border border-rose-200 bg-rose-50/60 rounded-md p-4 space-y-3">
          <p class="text-[10px] font-black uppercase tracking-wider text-rose-800">Priorité mission (staff)</p>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label for="mission-priority-level" class="block text-[9px] font-black uppercase tracking-wider text-slate-600 mb-1">Niveau</label>
              <select id="mission-priority-level" class="w-full bg-white border border-slate-200 text-slate-900 px-3 py-2 rounded-md text-sm">
                <option value=""<?= $missionPriorityLevelDefault === '' ? ' selected' : '' ?>>Aucun</option>
                <option value="critical"<?= $missionPriorityLevelDefault === 'critical' ? ' selected' : '' ?>>Critique</option>
                <option value="high"<?= $missionPriorityLevelDefault === 'high' ? ' selected' : '' ?>>Haute</option>
                <option value="standard"<?= $missionPriorityLevelDefault === 'standard' ? ' selected' : '' ?>>Standard</option>
              </select>
            </div>
            <div>
              <label for="mission-priority-role" class="block text-[9px] font-black uppercase tracking-wider text-slate-600 mb-1">Cible rôle</label>
              <input type="text" id="mission-priority-role" class="w-full bg-white border border-slate-200 text-slate-900 px-3 py-2 rounded-md text-sm" placeholder="all / command / instructors" value="<?= htmlspecialchars($missionPriorityRoleDefault, ENT_QUOTES, 'UTF-8') ?>">
            </div>
          </div>
          <div class="flex flex-col sm:flex-row sm:items-center gap-3">
            <label class="inline-flex items-center gap-2 text-[10px] font-bold text-slate-700">
              <input type="checkbox" id="mandatory-read" class="rounded border-slate-300 text-emerald-600"<?= $mandatoryReadDefault ? ' checked' : '' ?>>
              Lecture obligatoire (accusé de réception)
            </label>
            <input type="datetime-local" id="mandatory-read-due-at" class="bg-white border border-slate-200 text-slate-900 px-3 py-2 rounded-md text-sm" aria-label="Échéance de lecture obligatoire" value="<?= htmlspecialchars($mandatoryReadDueAtDefault, ENT_QUOTES, 'UTF-8') ?>">
          </div>
        </div>
        <?php endif; ?>
...
(function() {
  ...
  var canSetMissionPriority = <?= $canSetMissionPriority ? 'true' : 'false' ?>;
  var tags = [];
  var MAX_TAGS = 5;
  ...
  document.getElementById('new-topic-form').addEventListener('submit', function(e) {
    ...
    if (canSetMissionPriority) {
      var levelEl = document.getElementById('mission-priority-level');
      var roleEl = document.getElementById('mission-priority-role');
      var mandatoryReadEl = document.getElementById('mandatory-read');
      var dueAtEl = document.getElementById('mandatory-read-due-at');
      var missionPriorityLevel = levelEl ? (levelEl.value || '').trim() : '';
      var missionPriorityRole = roleEl ? (roleEl.value || '').trim() : '';
      var mandatoryRead = mandatoryReadEl ? mandatoryReadEl.checked : false;
      var mandatoryReadDueAt = dueAtEl ? (dueAtEl.value || '').trim() : '';
      if (missionPriorityLevel) createPayload.mission_priority_level = missionPriorityLevel;
      if (missionPriorityRole) createPayload.mission_priority_role = missionPriorityRole;
      if (mandatoryRead) createPayload.mandatory_read = true;
      if (mandatoryRead && mandatoryReadDueAt) createPayload.mandatory_read_due_at = mandatoryReadDueAt;
    }
