...
<div class="bg-slate-50 pb-16 sm:pb-24">
    ...
    <div class="mx-auto max-w-5xl space-y-10 px-4 -mt-6 sm:px-6 lg:px-8">
        ...
        <?php
        $renderEventBlock = static function (array $ev, array $checkFlags) use ($typeLabel, $rsvpLabel, $formatSlot, $typePillClass): void {
            $eid = (int) ($ev['id'] ?? 0);
            ...
            ?>
            <li class="group rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm ring-1 ring-slate-100 transition hover:border-emerald-200/80 hover:shadow-md sm:p-6">
                <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                    <div class="min-w-0 flex-1">
                        ...
                        <?php if (!empty($ev['description'])): ?>
                            <div class="mt-3 max-h-36 overflow-y-auto rounded-lg border border-slate-100 bg-slate-50/50 px-3 py-2 text-sm leading-relaxed text-slate-600 sm:max-h-none sm:border-0 sm:bg-transparent sm:px-0 sm:py-0"><?= nl2br(htmlspecialchars((string) $ev['description'], ENT_QUOTES, 'UTF-8')) ?></div>
                        <?php endif; ?>
                    </div>
                    <div class="shrink-0 rounded-xl border border-slate-100 bg-slate-50/90 px-4 py-3 text-sm lg:min-w-[200px] lg:text-right">
                        ...
                        <?php if (!empty($checked)): ?>
                            <p class="mt-2 text-xs font-semibold text-emerald-700">Présence enregistrée le <?= $formatSlot(is_string($checked) ? $checked : null) ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="mt-5 flex flex-col gap-4 border-t border-slate-100 pt-5 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
        ...
        <?php if ($pointagePast !== []): ?>
            <section class="scroll-mt-6" aria-labelledby="pointage-historique">
                ...
                <ul class="mt-5 divide-y divide-slate-100 overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-100">
                    <?php foreach ($pointagePast as $ev): ?>
                        ...
                        ?>
                        <li class="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-5">
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                            ...
                            <div class="flex shrink-0 flex-wrap items-center gap-2 sm:justify-end">
                                ...
                                <?php if ($didCheck): ?>
                                    <span class="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-900 ring-1 ring-emerald-200/80">
                                        <span class="h-1.5 w-1.5 rounded-full bg-emerald-500" aria-hidden="true"></span>
                                        Présence enregistrée
                                    </span>
                                <?php else: ?>
                                    <span class="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-500">
                                        Pas de pointage
                                    </span>
                                <?php endif; ?>
                            </div>
