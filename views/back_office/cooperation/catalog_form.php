...
<div class="max-w-2xl mx-auto px-6 py-10">
    <a href="<?= url('back-office/cooperation/catalog') ?>" class="text-sm font-medium text-slate-600 hover:text-slate-900 underline">← Retour</a>
    <h1 class="mt-4 text-2xl font-black text-slate-900"><?= $isEdit ? 'Modifier l’entrée locale' : 'Ajouter un type ou modèle local' ?></h1>
    ...
    <form method="post" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>" class="mt-8 space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div>
            ...
            <input type="text" id="slug" name="slug" required pattern="[a-z0-9_]{1,64}" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder="mon_modele_joint">
        ...
        <div>
            ...
            <textarea id="checklist_lines" name="checklist_lines" rows="4" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm"><?= htmlspecialchars($checkLines, ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>
