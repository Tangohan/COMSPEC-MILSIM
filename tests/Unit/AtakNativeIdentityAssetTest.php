...
final class AtakNativeIdentityAssetTest extends TestCase
{
    public function testNativeClientHasIndependentModAndDllIdentity(): void
    {
        ...
        $remoteSync = (string) file_get_contents($mod . '/Sources/addons/main/functions/network/fn_remoteSync.sqf');
        $pollUnits = (string) file_get_contents($mod . '/Sources/addons/main/functions/network/fn_pollUnits.sqf');
        $pollMarkers = (string) file_get_contents($mod . '/Sources/addons/main/functions/network/fn_pollMarkers.sqf');
        $pollOrders = (string) file_get_contents($mod . '/Sources/addons/main/functions/network/fn_pollOrders.sqf');
        $pollChat = (string) file_get_contents($mod . '/Sources/addons/main/functions/network/fn_pollChat.sqf');
        ...
        self::assertStringNotContainsString('comspec_overwatch_connect_fnc_', $remoteSync);
        self::assertStringContainsString('comspec_atak_native_fnc_pollUnits', $remoteSync);
        self::assertStringContainsString('["GetUnits",[]]', $pollUnits);
        self::assertStringContainsString('["GetMarkers",["world:" + worldName]]', $pollMarkers);
        self::assertStringContainsString('["GetOrders",', $pollOrders);
        self::assertStringContainsString('["GetChatMessages",', $pollChat);
        ...
        self::assertStringContainsString('ServePollGet("GetUnits"', $extension);
