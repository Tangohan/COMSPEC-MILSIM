...
final class ChangelogCatalogTest extends TestCase
{
    public function testHydrateKeepsPublishedReleasesAndHonestPipeline(): void
    {
        ...
        self::assertCount(8, $catalog['releases']);
        self::assertSame('2026-08-portal', $catalog['featured']['id']);
        self::assertTrue($catalog['featured']['featured']);
        self::assertSame(['atak', 'personnel', 'training', 'platform'], $catalog['featured']['categories']);
        self::assertContains('atak', $catalog['featured']['filter_groups']);
        self::assertContains('personnel', $catalog['featured']['filter_groups']);
        self::assertContains('training', $catalog['featured']['filter_groups']);
        self::assertContains('platform', $catalog['featured']['filter_groups']);
        self::assertNotSame('', $catalog['featured']['title']);
        self::assertNotSame('', $catalog['featured']['why']);
        self::assertCount(8, $catalog['featured']['features']);
