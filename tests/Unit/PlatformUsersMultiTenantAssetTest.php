...
{
    public function testOrphanFilterAndPersonDossierAreWired(): void
    {
        $root = dirname(__DIR__, 2);

        $repo = (string) file_get_contents($root . '/app/Repositories/UserRepository.php');
        self::assertStringContainsString('hasActiveNonDefaultMembershipPredicate', $repo);
        self::assertStringContainsString('shouldHideOrphanedPlatformAccounts', $repo);
        ...
        self::assertStringContainsString("u_live.status = 'active'", $repo);
        self::assertStringContainsString("t_live.slug <> 'default'", $repo);
    ...
    public function testDefaultListingHidesOrphansButInactiveFilterDoesNot(): void
    {
        $repo = (string) file_get_contents(dirname(__DIR__, 2) . '/app/Repositories/UserRepository.php');
        self::assertMatchesRegularExpression(
            '/function shouldHideOrphanedPlatformAccounts.*?status === \'active\'/s',
            $repo
        );
        self::assertStringContainsString("if (\$status === null || \$status === '' || \$status === 'active')", $repo);
    }
}
