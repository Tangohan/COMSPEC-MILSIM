...
window.ATAKLaserCodes = (function () {
  ...
  function fetchLaserCodes() {
    ...
    fetch(url, { credentials: 'include', cache: 'no-store' }).then(function (r) { return r.ok ? r.json() : null; }).then(function (data) {
      if (data == null) return;
      if (!Array.isArray(data)) return;
      codes = data;
      if (window.ATAKJTAC && window.ATAKJTAC.fillLaserSelect) {
        window.ATAKJTAC.fillLaserSelect();
      }
